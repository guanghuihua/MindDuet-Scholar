...
def em_canard_timeweighted(
    ...
    while t < t_stop:
        ...
        # Euler-Maruyama step: noise only on fast variable x
        trajectory_point_x += mu_1 * dt + eps * np.sqrt(dt) * np.random.randn()
        trajectory_point_y += mu_2 * dt
...
def main():
    ...
    ax1.set_title("EM Canard Density (Noise on Fast Variable)")
    plt.show()
