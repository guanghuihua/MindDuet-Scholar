...
def run_em_convergence(dt_list, gamma=1.0, sigma=1.0,
                       n_v_ref=40, n_steps=20_000_000, n_burnin=200_000):
    """
    固定足够长的轨迹（n_steps），扫描不同 dt，
    展示 EM 误差随 Δt 的收敛行为。
    网格固定为 n_v_ref，使空间投影误差远小于时间离散误差。
    """
    n_x    = n_v_ref ** 2
    x_grid = np.linspace(-4.0, 4.0, n_x)
    ...
    for dt in dt_list:
        ...
        print(f"  [EM conv]  dt={dt:.1e} err={err:.3e} t={t_em:.2f}s")
...
def plot_em_convergence(res_em, out_path):
    ...
    ax.set_title('EM method: temporal convergence\n'
                 f'(fixed $M={20_000_000:,}$ steps, grid $n_v=40$)', fontsize=13)
    ax.grid(True, which='both', ls='-', alpha=0.2)
    ax.legend(fontsize=10)
    fig.tight_layout()
    fig.savefig(out_path, dpi=300)
