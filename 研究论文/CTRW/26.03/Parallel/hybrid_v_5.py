...
def stationary_via_eigs(P: sp.csr_matrix) -> np.ndarray:
    """
    用 ARPACK 求转移矩阵特征值为 1 的特征向量（平稳分布）。
    对大矩阵自动放大 ncv 并在不收敛时回退到幂迭代。
    """
    N = P.shape[0]
    # ncv 需远大于 k，经验值：min(max(50, N//20), N-1)
    ncv = min(max(50, N // 20), N - 1)
    maxiter = max(10000, N * 5)

    try:
        vals, vecs = spla.eigs(
            P, k=1, which='LM',
            tol=1e-10,          # 对大矩阵稍放宽，避免死循环
            maxiter=maxiter,
            ncv=ncv,
        )
        pi = np.real(vecs[:, 0])
        ...
        # 部分收敛时 e.eigenvalues / e.eigenvectors 仍可用
        if len(e.eigenvectors) > 0:
            print(f"  ARPACK 部分收敛，使用已有结果（N={N}）")
            pi = np.abs(np.real(e.eigenvectors[:, 0]))
            pi /= pi.sum()
            return pi
        # 完全不收敛 → 回退到幂迭代
        print(f"  ARPACK 未收敛（N={N}），回退到幂迭代...")
        return power_iteration_sparse(P, max_iter=50000, tol=1e-11)
