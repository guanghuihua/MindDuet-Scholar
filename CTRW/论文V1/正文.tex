...
对于具有超线性增长和全局单侧 Lipschitz 连续漂移系数的随机微分方程，这种 Tamed Euler 方法以标准阶 $\frac{1}{2}$ 强收敛于精确解。并且模拟结果表明，这种显式强收敛的 Tamed Euler 方法比隐式 Euler 方法要快得多。2015 年，Mao \cite{mao2015truncated}对于具有
...
$$
d X(t)=\mu(X(t)) d t+\sigma(X(t)) d W_{t}
$$

提出了一种新的显式数值方法－截断 Euler－Maruyama 方法，并证明了该方法的强收敛性。随后在2016年，他 \cite{mao2016convergence} 给出了截断 Euler－Maruyama 方法的 $L^{q}(q \geq 2)$ 收敛速度，并表明在某些附加条件下，$L^{q}$ 收敛阶可以任意接近 $q / 2$ 。然而，截断函数存在一些限制，这些限制可能会迫使步长太小以致截断 Euler－Maruyama 方法不适用。2018 年， $\mathrm{Hu}, \mathrm{Li}$ 和 $\mathrm{Mao}$ \cite{Hu2018} 建立了不受这些限制的收敛速度，并研究了截断 Euler－Maruyama 方法的稳定性。但是以上方法对随机微分方程长时间的数值模拟效果并不好。

对于空间离散方法，2018年，Bou－Rabee和 Vanden－Eijnden \cite{bou2018continuous} 提出了随机微分方程的连续时间数值方法，这些方法是通过对与随机微分方程相关的 Kolmogorov方程进行空间离散得到的，这样得到的半离散方程产生一个马尔可夫跳跃过程，该过程可以使用蒙特卡罗方法精确实现。在这种构造中，近似的跳跃步长可以在空间上一致有界，这通常保证了这些方法在有限时间和长时间的随机微分方程模拟中都是数值稳定的。通过直接分析近似的无穷小生成元，证明了当随机微分方程的漂移项满足局部 Lipschitz 连续条件且弱耗散时，近似具有强随机 Lyapunov 函数，并且证明了近似在表示有限和无限时间期望值时是弱精确的，且与近似的无穷小生成元具有相同的精度。
...
其中 $\sigma=\operatorname{diag}\left(\sigma_{11}, \sigma_{22}, \ldots, \sigma_{n n}\right)$ 是噪声强度，并且对于 $i=1,2, \ldots, n, \sigma_{i i} \in \mathbb{R}^{+}$。当对方程（1．5）应用连续时间随机游走方法 $Q_{u}$（有限差分离散化）时漂移项会产生额外的扩散项，2021年， Li 和 $\mathrm{Zu}^{[25]}$（己投稿）提出了改进的 $Q_{u}$ 方法（本文中用 $Q_{u}^{L Z}$表示这种改进的 $Q_{u}$ 方法），通过减小式（1．5）中的 $\sigma$ 来补偿这种额外的扩散项。



在第一类方法中，Hutzenthaler等人在2012年提出了驯化Euler方法（Tamed Euler）。该方法在Euler–Maruyama方案的基础上对漂移项进行了“驯服”处理：在每个时间步，使用有界函数削弱漂移的大小，从而将数值增量中的漂移量限制在一个上界以内。这种处理避免了漂移过大导致的单步跳跃失控，从而抑制了解的发散。值得注意的是，Tamed Euler方法仍保持与EM相同的强收敛阶（$1/2$阶），适用于一侧Lipschitz的漂移和全局Lipschitz的扩散系数情形。随后，Mao等人在2015年提出了截断Euler–Maruyama方法（truncated EM），通过对漂移项$\mu(x)$和扩散项$\sigma(x)$实施适度的截断来处理超线性增长的情形。其核心思想是为数值方法引入一个依赖步长$\Delta$的阈值，将状态空间中超过阈值的区域“裁剪”掉，在计算时用阈值处的函数值替代原函数值。这样可强制将漂移和扩散限制在有界范围内，避免显式EM在超线性漂移下可能出现的爆炸。在局部Lipschitz条件及Khasminskii型线性增长条件下，截断EM方法被证明在均方意义下是收敛的；进一步地，如果扩散项满足多项式增长条件，还可以推得其“路径依赖”的强收敛性（即几乎必然收敛）。另外有研究指出，相较于驯化Euler，截断方法在相同误差控制下允许使用更大的步长，从而提高了效率。除了上述两种，Wei Liu等人提出的停止时间Euler方法通过引入随机停止时间，在数值解进入非物理区域（例如变量变为负值）之前终止模拟，从而保证了解的有界性和物理合理性。这进一步丰富了非Lipschitz情形下显式方法的改进手段。

在另一方向，Vanden-Eijnden等学者引入了空间离散的思想。他们不再沿用传统的时间步进，而是从SDE的无穷小生成元（一个二阶微分算子）入手，采用有限差分或有限体积等方法对其进行离散近似，从而构造出在离散状态空间上演化的马尔可夫过程。只要离散算子满足所谓的“可实现性”条件，即它可以解释为某离散Markov链的生成元（迁移率矩阵满足非负性和行和为零），那么便可以借助随机模拟算法（Stochastic Simulation Algorithm, SSA）来精确地模拟该Markov跳跃过程。这种方法的显著优点在于：数值解的空间步长是离散的且受控的，每次跳跃的幅度由网格间距$\delta x$决定，因此数值解始终被限制在SDE定义域内，不会越过物理边界条件；同时，由于跳跃速率可以随状态自适应调整，空间离散方法天然地对不同状态区域的刚性具有自适应性，在漂移变化剧烈的区域会发生更频繁的小步跳跃，在平稳区域则长时间驻留，这在一定程度上缓解了多尺度问题带来的困难。此外，通过精心设计跳跃率满足平衡条件，离散方案可以保证原SDE的平稳分布正是该马尔可夫链的稳定分布，从而理论上严格保证对稳态分布的采样。Bou-Rabee和Vanden-Eijnden在相关工作中分别提出了基于有限差分的“Qu”方案和基于有限体积的“Qc”方案，它们在弱意义下对SDE具有高阶准确性，并能较好保持SDE的几何性质。需要指出的是，这类连续时间随机行走（CTRW）方法在逼近漂移项时是通过泊松过程实现的：漂移引起的确定性运动被离散化为一系列随机跳跃及等待时间。这意味着漂移贡献被引入了一定的人工扩散（因为泊松过程的等待时间是随机的，会带来方差）。对于噪声强度较大的系统，这一影响可以忽略；但当原SDE扩散项很小（如噪声标准差与网格步长同阶甚至更小）时，这种人工扩散将显著扭曲数值解，导致模拟结果偏离真实行为。针对这一不足，祖建等人提出了一种改进的Q$\_u$跳跃格式。该方法在原有Qu方案基础上，引入了一个修正项来抵消漂移离散化所引入的有效扩散，等价于对跳跃过程附加了校正的扩散项。理论分析表明，Q$\_u$格式相较原始的Qu和Qc方案在精度上更胜一筹，特别是在“小噪声”情形下依然适用自如（而Qc方法在噪声很小时会失效）。进一步地，Q$\_u$方案在平均驻留时间（mean holding time）的渐近展开上具有良好的性质，可保证当网格加密时数值平均步长逼近解析值并达到高阶收敛。

综上所述，针对具有局部Lipschitz而非全局Lipschitz漂移的随机系统，当前主要有两类数值离散途径：一是改进的时间离散方法（如截断欧拉和驯化欧拉），二是基于生成元的空间离散方法（Qu/Qc跳跃方案及其改进）。两类方法各有优劣，但在具体应用中应如何抉择、它们在稳定性和效率方面孰优孰劣，目前缺乏系统的比较研究。而在实际数值模拟中，一个亟待解决的问题是：对于给定的SDE模型，哪种离散方式能以更小的误差或更高的效率获得可靠结果。为此，本文拟通过在若干典型随机动力系统上的实验，比较时间离散与空间离散两类方法在稳定性、精度、收敛性以及结构保持等方面的差异。特别地，我们将引入一个公平的比较准则：考察在数值解走过相同的空间距离时，两类方法各自所需的演化时间及计算代价，从而评估其效率和稳健性。这一准则等价于比较空间离散方法的平均停留时间与时间离散方法的等效步长所导致的推进速度，能够较直观地反映不同算法在相同距离尺度上的表现差异。

\subsection{本文的主要研究内容}

本文整体思路是，对于$dX_t = -X_t^3dt + \sqrt{2}dWt$这个方程，当$X$非常大的时候，系统主要由漂移项主导，给定两个位置$x_i,x_j$，认为$dX_t = -X_t^3dt$求解出来的运动时间就是精确的运动时间记为$t^e$，现在我想要用空间离散格式的mean holding time记为$t^u$和$t^e$比较，这个大概是一阶的，然后我想要获得如果我使用截断或者tamed的EM格式，来得到从$x_i$运动到$x_j$的时间，然后和$t^e$比较

本文的具体安排如下：第一章介绍课题背景和研究动机；第二章总结随机微分方程数值解的理论基础，包括解的存在唯一性、基本数值方法及其收敛性理论；第三章重点阐述常用的改进显式时间离散格式（截断Euler和驯化Euler）的原理与适用条件；第四章介绍通过离散生成元构造马尔可夫跳跃过程的空间离散方法，并描述Bou-Rabee与Vanden-Eijnden提出的Qu/Qc格式及祖建等人的改进方案的结构与性质；第五章给出数值实验，分别以一维立方振子模型和随机Canard慢快系统为例进行对比分析（在该部分中，每个模型先介绍方程和研究动机，实验图表与详细分析留待后续补充）；第六章综合比较两类方法的性能，从稳定性、精度、收敛阶及结构保持性等方面讨论各自的优劣，并结合模型特征进行评述；第七章总结全文并展望进一步的研究方向。
...
% \subsection{常见可解析的随机微分方程（SDE）}

%  常见可解析 SDE 一览表（含解法与分布）
...
% 在给定初值下，SDE解的存在性和唯一性是数值模拟的前提。经典Ito理论表明：如果漂移系数$\mu(x)$和扩散系数$\sigma(x)$满足以下条件之一组：

%  全局Lipschitz条件：存在常数\(K\_1>0\)，对任意\( x,y\in\mathbb{R}^n\)都有
%   $ \|\mu(x)-\mu(y)\| + \|\sigma(x)-\sigma(y)\| \le K_1 \|x-y\|,$

%  线性增长条件：存在常数\( K\_2>0\)，使得对任意\( x\)有
%   $\|\mu(x)\|^2 + \|\sigma(x)\|^2 \le K_2\,(1+\|x\|^2),$

% 则该SDE在全空间上存在唯一的强解，且解过程是适应的、具有右连续左极限（càdlàg）性质。这里的“强解唯一”指的是针对给定的概率空间和布朗运动轨道，解过程几乎必然唯一。这一定理为数值方法的设计提供了理论保障：在这些假设下，Euler–Maruyama离散方案得到的发展方程也可证实是均方收敛于真解的。

% 然而，实际模型中常常违背上述全局Lipschitz或线性增长条件。例如在物理、生物系统里漂移项\(\mu(x)\)往往是多项式或指数增长，当\( |x|\)较大时不满足线性增长，有时甚至在无界区域上失去Lipschitz连续性。经典理论难以直接适用于这类非全局Lipschitz情形。2002年，Higham、Mao和Stuart发表了开创性工作，将SDE数值解的强收敛理论扩展到漂移仅满足局部Lipschitz而不满足全局Lipschitz的情形。他们引入了一类弱于线性增长的条件（称为Khasminskii型条件），以确保SDE解的全局存在性：典型地，这类条件要求存在适当的Lyapunov函数\( V(x)\)，使得$\mathcal{L}V(x)$线性有界，从而推知解不会在有限时刻爆炸。基于此，Higham等证明：只要精确解和数值解的$p$阶矩在任意有限时间内有统一上界，那么显式Euler方法在局部Lipschitz条件下仍能以$1/2$阶强收敛于真实解。这一成果打开了在更一般假设下研究SDE数值解的大门。此后，大量文献围绕如何在放宽条件下确保数值方法的收敛与稳定展开，其中就包括前面提到的截断法、驯化法等改进方案的收敛性分析。
...
% 利用偏微分方程（PDE）可以从期望值的角度研究随机微分方程（2．3）的扩散过程。设 $f: \Omega \rightarrow \mathbb{R}$ 是一个二次可微函数。由随机微分方程（2．3）定义的扩散过程的无穷小生成元为
...
% 描述．Kolmogorov 方程（2．5）的解 $u(t, x)=\mathbb{E}_{x} f(X(t))$ ，其中 $\mathbb{E}_{x}$ 表示在 $X(0)=x$条件下 $X(t)$ 的期望。

% 还可以从偏微分方程角度来研究随机微分方程（2．3）解的动力学。设 $L^{*}$ 是 $L$的自伴算子，由


% \begin{equation*}
% L^{*} f(x)=-\sum_{1 \leq i \leq n} \frac{\partial\left(\mu_{i} f\right)(x)}{\partial x_{i}}+\sum_{1 \leq i, j \leq n} \frac{\partial^{2}\left(M_{i, j} f\right)(x)}{\partial x_{i} \partial x_{j}} \tag{2.6}
% \end{equation*}
...
% \begin{equation*}
% \frac{\partial p}{\partial t}(t, x)=L^{*} p(t, x), \quad p(0, x)=p_{0}(x) \tag{2.7}
% \end{equation*}
...
% 在本文中我们假设：\\
% 1．方程（2．3）存在唯一的扩散过程解 $\left\{X_{t}\right\}_{t \geq 0}$ ，且是一个马尔可夫过程。\\
...
% \begin{equation*}
% Q f(x):=\sum_{i=1}^{K} q\left(x, y_{i}(x)\right)\left(f\left(y_{i}(x)\right)-f(x)\right), \tag{2.8}
% \end{equation*}


% 其中 $q: \Omega \times \Omega \rightarrow[0, \infty)$ 是反应速率函数。假设 $Q$ 是无穷小生成元 $L$ 的好的近似，那么基于随机模拟算法（SSA）${ }^{[8]}$ 的连续时间随机游走方法描述如下。给定当前状态 $X(t)=x$.\\
% 第一步：通过生成参数为
...
% \begin{align*}
% Q_{u} f(x)= & \sum_{i=1}^{n}\left(\frac{\left(\tilde{\mu}(x)^{T} \sigma_{i}(x)\right) \vee 0}{h_{i}^{+}(x)}+\frac{1}{h_{i}^{+}(x) h_{i}(x)}\right)\left(f\left(x+h_{i}^{+}(x) \sigma_{i}(x)\right)-f(x)\right)  \tag{2.9}\\
...
% & +\frac{1}{h_{i}^{-}(x) h_{i}(x)} \exp \left(-\frac{h_{i}^{-}(x)}{2} \tilde{\mu}(x)^{T} \sigma_{i}(x)\right)\left(f\left(x-h_{i}^{-}(x) \sigma_{i}(x)\right)-f(x)\right) \tag{2.10}
% \end{align*}
...
% \begin{equation*}
% d X_{t}=\mu\left(X_{t}\right) d t+\sigma d W_{t} \tag{2.11}
% \end{equation*}
...
% \begin{align*}
% Q_{u} f(x)= & \sum_{i=1}^{n}\left(\frac{\mu_{i}(x) \vee 0}{h}+\frac{M_{i i}}{h^{2}}\right)\left(f\left(x+h e_{i}\right)-f(x)\right)  \tag{2.12}\\
% & +\left(-\frac{\mu_{i}(x) \wedge 0}{h}+\frac{M_{i i}}{h^{2}}\right)\left(f\left(x-h e_{i}\right)-f(x)\right), \\
% Q_{c} f(x)= & \sum_{i=1}^{n} \frac{M_{i i}}{h^{2}} \exp \left(\frac{\mu_{i}(x) h}{2 M_{i i}}\right)\left(f\left(x+h e_{i}\right)-f(x)\right)  \tag{2.13}\\
...
% \begin{align*}
% Q_{u}^{L Z} f(x)= & \sum_{i=1}^{n}\left(\frac{\mu_{i}(x) \vee 0}{h}+\frac{M_{i i}}{h^{2}}\right)\left(f\left(x+h e_{i}\right)-f(x)\right)  \tag{2.14}\\
% & +\left(-\frac{\mu_{i}(x) \wedge 0}{h}+\frac{M_{i i}}{h^{2}}\right)\left(f\left(x-h e_{i}\right)-f(x)\right)
% \end{align*}
...
% & \quad q_{i 1} \leftarrow\left(\mu_{i}(X) \vee 0\right) / h+M_{i i} / h^{2}, q_{i 2} \leftarrow\left(-\mu_{i}(X) \wedge 0\right) / h+M_{i i} / h^{2}  \tag{2.15}\\
...
% & Q_{u}^{L Z} \text { 步骤 }: M_{i i} \leftarrow 0.5 *\left(\sigma_{i i}^{2}-\left|\mu_{i}(X)\right| h\right) \vee 0  \tag{2.16}\\
% & \quad q_{i 1} \leftarrow\left(\mu_{i}(X) \vee 0\right) / h+M_{i i} / h^{2}, q_{i 2} \leftarrow\left(-\mu_{i}(X) \wedge 0\right) / h+M_{i i} / h^{2} \tag{2.17}
% \end{align*}
...
% \begin{equation}
%     E|X_n - X(\tau)| \leq C \Delta t^\gamma \tag{5.1}
% \end{equation}
...
% \begin{equation}
% e^{\text{strong}}_{\Delta t} := E|X_L - X(T)|, \quad \text{其中 } L\Delta t = T, \tag{5.2}
% \end{equation}

% 它表示EM在强意义下的末端点误差。如果 (5.1) 在任意固定点成立，且 $\gamma = \tfrac{1}{2}$，那么它在末端点也成立，因此有

% \begin{equation}
% e^{\text{strong}}_{\Delta t} \leq C \Delta t^{1/2}, \tag{5.3}
% \end{equation}
...
% 虽然强收敛的定义 (5.1) 涉及期望，但它对单次模拟也有意义。Markov不等式指出：若随机变量 $X$ 的期望有限，则对任意 $a>0$，有
...
\section{空间离散方法的平均首达时间分析}

考虑 (2.1.2) 的一个特例：
\begin{align}
    dX_t = \mu(X_t)\,dt + \sqrt{2}\,dW, \quad X(0) \in \mathbb{R}.
\end{align}
...
因为网格间距很小，可以先把 integrand $\frac{1}{\mu(x)}$ 在 $x_i$ 附近展开,
...
下面是对你给出的段落的「整理 + 翻译 + 术语说明」。我保留了原始符号，并用更统一的记号（将 “t e” 写成 $t^{e}$）以便阅读。



我们将把精平均停留时间 $t^{e}$ 与由两个离散生成子 $\widetilde{Q}_u$（式 (3.1.1)）和 $\widetilde{Q}_c$（式 (3.1.2)）所预测的平均停留时间进行比较。

由式 (3.1.1)，$\widetilde{Q}_u$ 的平均停留时间可写为
...
这表明Qu离散方案在平均速度（漂移导致的推进速度）上具有二阶精度，即网格步长减半将使离散跳跃过程的推进速度更逼近真实SDE速度。类似地，Qc方案在这方面也具有二阶精度，但由于Qc在处理非平衡态漂移时会引入一些偏差，当扩散很小时Qc的精度和适用性不如Qu方案。前述祖建等提出的Q$\_u$改进方案正是为了解决小扩散情形下Qu/Qc方法的失真问题，它在离散过程中添加了一个修正扩散项补偿漂移的不确定性。通过这一调整，Q$\_u$方案能够在扩散项很弱甚至趋零时依然给出合理的数值模拟结果，而原始Qu方案在此极端条件下会由于泊松驱动的方差占主导而产生显著误差。值得一提的是，Q$\_u$方法在保持弱收敛精度的同时提高了稳定性，其跳跃过程对噪声强度具有更好的适应性。

\textbf{总的来说，空间离散类方法（Qu、Qc及其改进）最大的优点在于结构保持：数值解严格服从马尔可夫过程的性质，因此天然满足非负性、有界性等物理约束，并且理论上能够实现对平衡态的精确采样。这对于带有边界条件或不变集的SDE尤为重要（如具有反应灭绝态、生物种群下界的模型等，空间离散法保证数值解不会跑到负值或无意义区域）。另一方面，其不足也很明显：首先，在高维系统上构造离散网格将遭遇“维数灾难”，状态数随维度指数增长使得该方法目前主要限于低维应用；其次，相较时间步进方法，空间离散法的实现较复杂且每一步并非固定时间推进，这给误差分析和实现都带来难度；再次，当原系统扩散强且远离平衡时，时间离散方法往往能以较大步长稳定模拟，而空间离散法可能需要非常细的网格才能捕捉剧烈变化，导致计算量增加。因此，在具体问题中需要权衡两类方法的取舍，并可能针对模型特性选择最适合的方案。下一章我们将通过具体算例，更直观地观察改进的时间离散方法与空间离散方法在模拟效果上的差异。}
...
截断法的基本思想是：在每一步积分时，将漂移和扩散函数限制在一个有界范围内，从而避免过大的函数值导致数值解跳动失控。具体做法是选取一个阈值$R = R(h) > 0$，它可以依赖于时间步长$h=\Delta t$（一般取$R(h)$随$h$缩小，使步长越大时截断越“保守”）。然后定义截断漂移函数$\mu^h(x)$和截断扩散$\sigma^h(x)$如下：对任意状态$x\in\mathbb{R}^n$,

$$
    \mu^h(x) =  
    \begin{cases}  
        \mu(x), & \|x\| \le R(h);\\  
        \mu\!\Big(R(h)\frac{x}{\|x\|}\Big), & \|x\| > R(h),  
    \end{cases}  
$$

并类似地$\sigma^h(x) = \sigma(x)$当$|x|\le R(h)$，超过阈值时将$x$径向投影到球面$|x|=R(h)$再取$\sigma$值。简言之，当状态变量进入阈值半径之外时，我们将其“拉回”半径$R(h)$处，以截断方式限制$\mu$和$\sigma$的大小。基于截断后的$\mu^h,\sigma^h$，每一步使用Euler–Maruyama更新：

$$
    X_{n+1} \;=\; X_n + \mu^h(X_n)\,\Delta t + \sigma^h(X_n)\,\Delta W_n,  
$$

其中$\Delta W\_n$是时间步长$\Delta t$内的布朗增量。由于$\mu^h,\sigma^h$在全空间上满足全局Lipschitz且线性有界，上述离散迭代在理论上等价于对一个满足经典条件的“修正SDE”进行EM模拟，因此其稳定性和收敛性可以被严格证明。Mao (2015)的结果表明：在漂移满足局部Lipschitz及线性增长（或Khasminskii）条件、扩散满足局部Lipschitz且线性增长的前提下，截断EM解是均方渐近稳定的，并且强收敛阶仍为$1/2$。特别地，只要阈值半径的选择满足适当的比例（例如$R(h)\sim h^{-\alpha}$对某个$\alpha>0$），截断不会降低收敛阶。后续研究中，Mao等进一步给出了截断法在更广泛条件下的收敛率以及均值稳定性结论。总的来说，截断Euler方法是一种简单有效的改进策略，它通过在数值上“温和地”削平漂移和扩散曲线的尖峰，成功地将许多非全局Lipschitz的SDE纳入了显式仿真的可行范围。
...
\section{时间离散（Tamed EM）下的平均首达时间：在 $\boldsymbol{X}$ 充分大时的解析表述与比较}
...
为与空间离散法“公平比较”，以下均在固定的空间距离
$L:=x_i-x_j>0$
上评估平均首达时间。

\subsection{Tamed EM 格式与等效漂移}
...
例如在你经常讨论的 $\mu(x)=-x^3$ 且 $x>0$ 的区间上，$\widetilde{\mu}_\Delta(x)\approx -\,\Delta^{-1}$（速度饱和为常数负值）。

\subsection{平均首达时间的 PDE 与显式积分公式（Dynkin 公式）}
把 Tamed EM 看作等效 SDE

$$
dX_t=\widetilde{\mu}_\Delta(X_t)\,dt+\sqrt{2}\,dW_t,
$$
...
\subsection{在 $\boldsymbol{X}$ 充分大时的渐近（驯化区）}
...
\paragraph{对 $\mu(x)=-x^3$ 的下一阶修正（可选）。}
利用恒等分解
...
\subsection{与 $\boldsymbol{\mathbb{E}[\tau]}$ 及漂移主导时间 $\boldsymbol{t^e}$ 的比较}
...
\subsection{驯化Euler方法（Tamed EM）}
“驯化”方法由Hutzenthaler等人在2012年提出。与截断法直接截平函数不同，Tamed Euler方法是在Euler增量中加入一个非线性衰减因子，使得当漂移$\mu(X\_n)$很大时，其贡献相对减弱。具体实现上，Tamed EM修改了漂移项的计算公式：

$$
    X_{n+1} \;=\; X_n \;+\; \frac{\mu(X_n)}{\,1 + \Delta t\,\|\mu(X_n)\|\,}\,\Delta t \;+\; \sigma(X_n)\,\Delta W_n~,  
$$

其中$|\mu(X\_n)|$表示某种范数（对标量SDE可直接取绝对值）。上述更新式中，将原来的$\mu(X\_n)\Delta t$替换为$\displaystyle \frac{\mu(X\_n)}{1+\Delta t,|\mu(X\_n)|}\Delta t$。当$\mu(X\_n)$较小时，该修正因子$\frac{1}{1+\Delta t,|\mu(X\_n)|}$近似为1，方法几乎等价于标准Euler；但当$\mu(X\_n)$非常大时，因子趋近于$\frac{1}{\Delta t,|\mu(X\_n)|}$的倒数量级，从而显著减小了实际加入的漂移增量。形象地说，Tamed Euler在每一步“驯服”了过于猛烈的漂移驱动，确保数值解不会因为单步推进过大而发散。这样的处理保持了方法的显式和易实现特性，同时避免了对漂移函数进行生硬裁剪（漂移函数本身并未被修改或截断，而是动态地调节步长作用效果）。Hutzenthaler等证明了：对于漂移满足单边Lipschitz（one-sided Lipschitz）条件、扩散满足线性增长条件的SDE，Tamed Euler方法几乎必然有解且均方稳定，并且强收敛阶同样为$1/2$。此后的一系列工作（如Sabanis 2016、Wang和Gan 2013等）进一步验证和拓展了驯化思想，包括推广到更高阶方法（如Tamed Milstein方法）以及分析其在收敛和稳定性上的细节。总的来看，Tamed Euler提供了另一种应对超线性漂移的有效手段，在保持计算简洁性的同时，大幅提升了数值解轨道的稳定性。

需要指出的是，截断方法与驯化方法各有适用场景：截断法由于实际修改了漂移和扩散函数的形式，因此在涉及保序（如保持解的非负性）或物理约束时更具优势——通过选择合适的截断半径，可以确保数值解不进入非法区域；而驯化法不改变解的方向，仅缩小步长效应，因而更适合不希望“篡改”模型函数形式但又需要控制步长影响的情况。此外，停止时间修正等其他改进方法也可以视为对截断思想的变体：它通过随机判定来提前终止模拟，从另一个角度确保了解不爆炸。综上，这些改进显式方法为非Lipschitz连续系数SDE的数值解提供了行之有效的方案。它们的数值稳定性和精度将在第五章的数值实验中得到进一步对比和验证。
...
% 为比较基于时间离散和空间离散的两类数值格式的实际表现，我们选择了两个具有代表性的随机动力系统作为测试模型，并设计相应的数值实验。实验将在每个模型上同时应用改进的Euler方法（截断EM和Tamed EM）和离散生成元的跳跃方法（Qu/Qc或Q$\_u$），比较它们在相同条件下的稳定性、精度和效率差异。需要说明的是，出于版面和后续完善的考虑，以下每个子节仅给出模型方程及实验方案说明，具体的模拟结果图表与详细分析将留待后续补充完善。
