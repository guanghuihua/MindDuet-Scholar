...
Computation matching:
    ...
    - Only EM/SSA pairs with sufficiently small relative work gap are accepted.
"""
...
def filter_valid_pairs(pairs, max_gap_rel: float):
    return [pair for pair in pairs if pair[2] <= max_gap_rel]
...
def plot_results(results, pairs, target_work: float, out_dir: Path) -> None:
    ...
    if pairs:
        ...
        axes[1].text(
            0.5,
            0.5,
            f"No valid EM/SSA pairs\nwith work gap <= {100 * MAX_PAIR_GAP_REL:.0f}%",
            ha="center",
            va="center",
    ...
    axes[1].set_title("Valid matched-work EM vs SSA pairs")
...
def print_pairs(pairs) -> None:
    if not pairs:
        print(
            "\nNo valid EM/SSA pairs found under the current work-gap threshold "
            f"({100 * MAX_PAIR_GAP_REL:.0f}%)."
        )
        print("Add more dt / h_y values to create closer matched-work comparisons.")
        return
    print(
        "\nValid matched-work EM vs SSA pairs "
        f"(work gap <= {100 * MAX_PAIR_GAP_REL:.0f}%):"
    )
...
def main():
    ...
    pairs = filter_valid_pairs(build_pairs(results), MAX_PAIR_GAP_REL)
