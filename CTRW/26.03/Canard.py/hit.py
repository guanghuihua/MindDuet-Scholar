...
============================
比较 Euler-Maruyama (EM) 与 CTMC/SSA 两种数值格式在 canard 系统中
轨迹沿不稳定慢流形爬行、到达目标点的统计行为。

系统方程（FitzHugh-Nagumo / van der Pol canard 系）：
    dx = (y - x^3/3 + x) / delta * dt + eps * dW_x
    dy = (a - x)           * dt + eps * dW_y
    delta = 0.1,  a = a_canard ≈ 0.875（最大 canard 参数）

实验逻辑：
    每次试验从右侧稳定分支出发，轨迹爬上不稳定分支后，
    统计能到达目标点 x* 的次数及脱落位置分布。
"""
...
N_TRIALS   = 1000     # number of trials per scheme
